...
func (kp *Ed25519KeyPair) Sign(message []byte) ([]byte, error) {
	...
	// Standard Ed25519 signing - sign the RAW message, not a pre-hashed version
	// Ed25519 handles SHA-512 hashing internally as part of the signature algorithm
	// Pre-hashing would be double-hashing and would produce incorrect signatures
	signature := ed25519.Sign(ed25519.PrivateKey(kp.privateKey.Bytes()), message)

	Debug("Signing with Ed25519 keypair")
	Debug("Signing hash with Ed25519")
	Debug("Ed25519 signature created successfully")
...
// Uses standard Ed25519 verification which handles SHA-512 hashing internally.
func (kp *Ed25519KeyPair) Verify(message, signature []byte) bool {
	if kp.publicKey == nil {
		return false
	}

	// Standard Ed25519 verification - verify against RAW message, not pre-hashed
	// Ed25519 handles SHA-512 hashing internally as part of the verification algorithm
	pubKeyBytes := kp.publicKey.Bytes()
	Debug("Verifying Ed25519 signature hash")

	verified := ed25519.Verify(ed25519.PublicKey(pubKeyBytes), message, signature)
